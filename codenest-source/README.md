...
10. 点击 `Save changes` 后，文字、图片链接和本地上传图片会保存在当前浏览器中，刷新页面仍会保留。
...
本地上传的图片会压缩后保存在当前浏览器的 IndexedDB 中，文字和公开图片链接另有轻量备份。它们不会自动上传到服务器。需要让所有访问者长期看到同一份新内容时，请使用公开图片 URL，并替换项目中的默认配置后重新部署到 GitHub Pages。
